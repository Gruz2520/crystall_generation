...
import numpy as np
from scr.slices.tobascco_net import Net, SystreDB
from scr.slices.config import OFFSET, LJ_PARAMS_LIST, PERIODIC_DATA
import math
import tempfile
import json
...
class SLICES:
    ...
    def get_inner_p_target(self, bond_scaling=1.05):
        ...
        """
        nbf, blist = self.get_nbf_blist()
        temp_dir = tempfile.TemporaryDirectory(dir="D:/Projects/projects_python/genCry/dev/shm")
        try:
            with open(temp_dir.name+'/testBonds_cut.top','w') as f:
                f.write(nbf)
            subprocess.call(os.environ["XTB_MOD_PATH"]+' --gfnff testBonds_cut.top --wrtopo blist,vbond,alist,vangl', \
